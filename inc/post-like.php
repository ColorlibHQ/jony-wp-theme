...
add_action( 'wp_enqueue_scripts', 'jony_sl_enqueue_scripts' );
function jony_sl_enqueue_scripts() {

	wp_enqueue_script( 'jony-post-likes', JONY_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'jony-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'jony' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_jony_process_simple_like', 'jony_process_simple_like' );
add_action( 'wp_ajax_jony_process_simple_like', 'jony_process_simple_like' );
function jony_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'jony' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'jony' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'jony' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'jony' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'jony' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'jony' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'jony_show_user_likes' );
add_action( 'edit_user_profile', 'jony_show_user_likes' );
function jony_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'jony' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'jony' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
