...
final class Jony {
	...
	private function setup() {
		...
		$enqueu          = new jony_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->jony_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'jony_content_width', 751 );
		...
		load_theme_textdomain( 'jony', JONY_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'jony_site_logo_83x39', 83, 39, true );

		// Personal img thumb size
		add_image_size( 'jony_personal_img_487x762', 487, 762, true );

		// Portfolio img thumb sizes
		add_image_size( 'jony_portfolio_img_445x394', 445, 394, true );
		add_image_size( 'jony_portfolio_img_445x553', 445, 553, true );
		add_image_size( 'jony_portfolio_single_img_950x575', 950, 575, true );

		// Testimonial client thumbnail size
		add_image_size( 'jony_client_thumb_68x68', 68, 68, true );

		// Latest post thumbnail size
		add_image_size( 'jony_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'jony_single_blog_750x375', 750, 375, true );
		add_image_size( 'jony_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu'		=> esc_html__( 'Primary Menu', 'jony' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = JONY_DIR_CSS_URI;
		$jsPath  = JONY_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'jony' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate jony theme customizer
		$jony_theme_customizer = new jony_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'jony_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'jony-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'jony_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'jony-elementor-default-style',  JONY_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->jony_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'jony_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'jony-elementor-notice', JONY_DIR_JS_URI.'jony-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'jony-elementor-notice',
				'jonyElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'jony-elementor-notice-nonce' ),
				)
			);
		}
